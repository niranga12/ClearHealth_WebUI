...
function ActionHealthSystem({ row }) {
  ...
  useEffect(() => {
    const fetchPermission = async () => {
      let EditPermission = await PermissionButton(
        ScreenPermissions.HealthSystem,
        ButtonPermissions.EditHealthSystem,
        permissionList
      )
      setIsEditHealthPE(EditPermission)

      let DeletePermission = await PermissionButton(
        ScreenPermissions.HealthSystem,
        ButtonPermissions.DeleteHealthSystem,
        permissionList
      )
      setIsDeleteHealthPE(DeletePermission)

      let ViewHosptialPermission = await PermissionButton(
        ScreenPermissions.HealthSystem,
        ButtonPermissions.ViewHealthSystemHospital,
        permissionList
      )
...
const HealthTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        dispatch(loaderShow())
        // button Permission
        let Permission = PermissionButton(
          ScreenPermissions.HealthSystem,
          ButtonPermissions.AddHealthSystem,
          permissionList
        )
    ...
    () => [
      {
        ...
        Cell: ({ value }) => <h5 className="font-weight-normal text-black ml-4 max-health-name"> {value} </h5>
      },
      {
        Header: 'Address',
  ...
  const sortingHandler = (sorting) => {
    if (sorting.length > 0) {
      let result = {
        ...searchQuery,
        orderBy: sorting[0].id ? sorting[0].id : '',
