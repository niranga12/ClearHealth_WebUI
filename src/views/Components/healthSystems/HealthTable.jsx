...
import { getHealthSystemList, getHealthSystemListCount } from "src/service/healthsystemService";
...
function CellContract({row}) {
  ...
  <div className="rectangle-intable"> <span className="fa fa-phone text-health-icon pr-1"></span> {row.original.contactNumber}</div>
  <div className="rectangle-intable"> <span  className="fa fa-envelope text-health-icon pr-1"></span> {row.original.contactElectronicAddress}</div>
  </>
}
...
function CellAddress({row}) {
  return <>
  <div className="max-celladdress">{row.original.primaryAddress1 } , {row.original.primaryAddress2 } ,{row.original.primaryCity } , </div>
  <div className="max-celladdress">{row.original.primaryState } ,{row.original.primaryZip } </div>
  <div className="rectangle-intable"> <span className="fa fa-phone text-health-icon pr-1"></span> {row.original.phoneNumber}</div>
    ...
    () => [
      {
        ...
        Cell: ({value}) => (<h6 className="mt-4 font-weight-bold"> {value} </h6>)
      },
      {
        Header: "Address",
      ...
      },
      {
        ...
        Cell: ({value}) => (<div ><span  className="fa fa-ellipsis-h"></span></div>)
      },
    ],
    []
  );
...
<>
  ...
  < AdminHeaderWithSearch showCount={count}  handleSearchChange={searchTextChange } handleAddNew={addNewHeathSystem} placeholder="&#xF002; Search Patient name order no" buttonTitle="New Health System" />
    <div className="LatoRegular">
      ...
      <HealthPagination handlePageChange={pageChange} countPage={page}/>
