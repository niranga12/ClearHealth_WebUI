...
const HospitalProviderTable = () => {
  ...
  useEffect(() => {
    ...
    let Permission = PermissionButton(
      ...
      ButtonPermissions.HospitalAddProviders,
      permissionList
    )
  ...
  const ProviderActions = ({ row }) => {
    ...
    useEffect(() => {
      let Permission = PermissionButton(
        ...
        ButtonPermissions.AddProviderProcedures,
        permissionList
      )
      setAddPE(Permission)

      let deletePermission = PermissionButton(
        ScreenPermissions.Providers,
        ButtonPermissions.ViewProviderProcedures,
        permissionList
      )
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <h5 className="font-weight-normal text-black"> {row.original.livecount ? row.original.livecount : 0} </h5>
        ),
        disableSortBy: true
      },
      {
