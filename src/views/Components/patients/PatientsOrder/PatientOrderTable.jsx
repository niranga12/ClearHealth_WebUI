...
const viewInvoice = ({ row }) => {
  ...
  return (
    <>
      <div className="rectangle-invoice" onClick={redirectAccount}>
        View Invoice
      </div>
    </>
  )
  ...
  const columns = useMemo(
    () => [
...

      {
        ...
        accessor: 'orderDate',
        Cell: ({ row }) => (
          <div className="font-weight-normal text-black "> {moment(row.original.orderDate).format('MM-DD-YYYY')} </div>
        )
      },
      {
