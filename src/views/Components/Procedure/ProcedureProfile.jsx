...
const ProcedureProfile = () => {
  ...
  const searchTextChange = (e) => {
    ...
    let result = textData.filter(
      (x) => x.description.toLowerCase().includes(searchText) || x.code.toLowerCase().includes(searchText)
    )
    ...
    <>
      ...
      <div className="card  cover-content pt-2 ">
        <AdminHeaderWithSearch
          title={providerName}
          handleAddNew={save}
          buttonDisable={savePrcoedureData.length < 1}
          buttonTitle="Save"
          iconShow={false}
          handleSearchChange={searchTextChange}
          selectionList={ServiceType.Types}
          handleDropDownChange={dropDownChange}
          placeholder="Search here.."
          selectionTitle="Service Type"
          subHeader="Provider Name"
        />
        <div className="divider m-1"></div>
        <div className="row">
          <div className="col-md-4 ">{procedureData && <DataTable columns={columns} data={procedureData} />}</div>
