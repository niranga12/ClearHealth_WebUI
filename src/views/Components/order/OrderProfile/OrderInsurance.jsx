...
const OrderInsurance = ({ defaultValues, isEdit = false, handleInsuranceForm, patientDetail }) => {
  ...
  const verifyInsurance = async () => {
    try {
      ...
      let data = {
        ...formValue,
        payerId: selectedPayerId?.payerID,
        patientBirthDate: patientDetail.dateOfBirth,
        patientGender: patientDetail.gender
      }

      let result = await verifyInsuranceDetails(data)
      if (
        result.data.data.statusCode == 400 ||
        result.data.data.statusCode == '19' ||
        result.data.data.statusCode == '3'
      ) {
        setAlertMessage(result.data.data.message)
        setfieldMessage(
          result.data.data.data.map(function (obj) {
            ...
            return (
              <div>
                <span>{obj.field}</span> <span>{obj.field != undefined && ':'}</span>
    ...
    <div>
      ...
      <div className="row">
        <div className="col-md-4">
          <div className="form-group">
            ...
            <AsyncSelect
              cacheOptions
              defaultOptions
              getOptionLabel={(e) => e.payerID + ' - ' + e.payerName}
              getOptionValue={(e) => e.payerID}
              loadOptions={loadOptions}
              {...register('insurance.payerId')}
        ...
        <div className="col-md-4">
          <div className="form-group">
            <label className="form-text">
              Relationship to Subscriber <span className="text-danger font-weight-bold ">*</span>
            </label>
            <select
              name=""
              id=""
              className="form-control-sm"
              {...register('insurance.subscriberRelationship')}
              onBlur={() => setstateChange(!stateChange)}
            >
      ...
      <div className="row">
        ...
        <div className="col-md-4">
          <div className="form-group">
            <label className="form-text">
              Provider NPI<span className="text-danger font-weight-bold ">*</span>
            </label>
            <input
              className="form-control-sm"
              value="1104918564"
              type="text"
              {...register('insurance.providerNpi')}
              readOnly={true}
              onBlur={() => setstateChange(!stateChange)}
        ...
        <div className="col-md-4">
          <div className="form-group">
            <label className="form-text">Group ID</label>
            <input
              className="form-control-sm"
              type="text"
        ...
        <div className="col-md-4">
          <div className="form-group">
            ...
            <input
              className="form-control-sm"
              type="text"
              {...register('insurance.patientFirstName')}
              readOnly={isEdit}
              onBlur={() => setstateChange(!stateChange)}
            />
        ...
        <div className="col-md-4">
          <div className="form-group">
            ...
            <input
              className="form-control-sm"
              type="text"
              {...register('insurance.patientMiddleName')}
              readOnly={isEdit}
              onBlur={() => setstateChange(!stateChange)}
            />
        ...
        <div className="col-md-4">
          <div className="form-group">
            ...
            <input
              className="form-control-sm"
              type="text"
              {...register('insurance.patientLastName')}
              readOnly={isEdit}
              onBlur={() => setstateChange(!stateChange)}
            />
