...
const GlobalToolTable = ({ filterDetail }) => {
  ...
  const CalculationPackage = (enhancementPercentage) => {
    let updatedData = globalPackage.map((x) => {
      let packagesTotal =
        Number(x.optimizedFacilityRate) +
        Number(x.optimizedPhysicianRate) +
        Number(x.anesthesiaRate) +
        Number(x.pathologyRate)
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right "> {CurrencyFormat(Number(row.original.optimizedFacilityRate), true)} </div>
        )
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right "> {CurrencyFormat(Number(row.original.optimizedPhysicianRate), true)} </div>
        )
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right "> {CurrencyFormat(Number(row.original.anesthesiaRate), true)} </div>
        )
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right "> {CurrencyFormat(Number(row.original.pathologyRate), true)} </div>
        )
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right "> {CurrencyFormat(Number(row.original.clearTransactionalFee), true)} </div>
        )
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right "> {CurrencyFormat(Number(row.original.packagePrice), true)} </div>
        )
      }
    ],
    []
