...
import {
  FacilityPackageField,
  FeeScheduleSingleEdit,
  PackageItems,
  PhysicianPackageField,
  ServiceMsg
...
const PricingSingleEdit = ({ row, category = null }) => {
  ...
  useEffect(() => {
    if (
      (filterDetail && filterDetail?.clPrice == 'Auto' && filterDetail?.enhancementOn == '') ||
      (filterDetail?.clPrice == 'Auto' && filterDetail?.enhancementRate == '')
    ) {
      setisEdit(false)
    } else {
      setisEdit(true)
  ...
  const hospitalRateChange = () => {
    if (filterDetail?.clPrice == 'Auto' && filterDetail?.enhancementOn == FeeScheduleSingleEdit.collectionAmount) {
      ...
      let newClearFees =
        Number(collectionAmount) * (Number(filterDetail?.enhancementRate) / 100) + Number(collectionAmount)
      setValue('priceTool.clearOptimizedPrice', String(newClearFees))
    }
  }
    ...
    return (
      <>
        <div className="col-md-6">
          ...
          <input
            type="number"
            className={`form-control-sm col-9 ${filterDetail?.clPrice == 'Auto' ? 'avoid-clicks' : ''} `}
            {...register('priceTool.estimatedPayLaterPrice')}
          />
        </div>
      </>
    )
    ...
    <>
      ...
      <CModal size={'lg'} show={editMode} onClose={setEditMode} closeOnBackdrop={false}>
        <CModalHeader closeButton>
          {' '}
          <h5 className="font-weight-bold">Update FeeSchedules</h5>{' '}
        </CModalHeader>
        <CModalBody>
            ...
            <div className="row">
              <div className="col-md-6">
                ...
                <input
                  ...
                  {...register('priceTool.mediacareRate')}
                  onBlur={mediaRateChange}
                  disabled={mediacareRateDisable}
                />
              </div>
              <div className="col-md-6">
                <label htmlFor="">
                ...
                <input
                  type="number"
                  className=" form-control-sm col-9"
                  {...register('priceTool.collectionAmount')}
                  onBlur={hospitalRateChange}
                  disabled={collectionAmountDisable}
                />
              </div>
            </div>
            <div className="row">
              ...
              <div className="col-md-6">
                ...
                <input
                  type="number"
                  className={`form-control-sm col-9 ${filterDetail?.clPrice == 'Auto' ? 'avoid-clicks' : ''} `}
                  {...register('priceTool.clearOptimizedPrice')}
                />
              </div>
            </div>
          </form>
