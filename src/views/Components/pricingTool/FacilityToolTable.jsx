...
const FacilityToolTable = ({ filterDetail }) => {
  ...
  const CalculationPackage = (enhancementPercentage, enhancementField, facilityDataDetail) => {
    let updatedData = facilityDataDetail.map((x) => {
      let newClearFees =
        Number(x[enhancementField]) * (Number(enhancementPercentage) / 100) + Number(x[enhancementField])
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right pr-3"> {CurrencyFormat(Number(row.original.medicareRate), true)} </div>
        )
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right pr-3"> {CurrencyFormat(Number(row.original.hospitalCollectionFee), true)} </div>
        )
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right pr-3"> {CurrencyFormat(Number(row.original.estimatedPayLaterPrice), true)} </div>
        )
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => (
          <div className="text-right pr-3"> {CurrencyFormat(Number(row.original.clearOptimizedFee), true)} </div>
        )
      },
